...
// Initialize clients with Kenyan environment variables
...
// Test data for Kenyan phone numbers and transactions
const paymentDataReceive = {
  from: process.env.PHONE_NUMBER, // input_CustomerMSISDN (Kenyan phone number)
...
const paymentDataSend = {
  to: process.env.PHONE_NUMBER, // input_CustomerMSISDN (Kenyan phone number)
  reference: '11114' + Math.floor(Math.random() * 100), // input_ThirdPartyReference
  transaction: 'T12344CC', // input_TransactionReference
  amount: '10', // input_Amount
};
...
const paymentDataBusiness = {
  to: '979797', // input_ReceiverPartyCode (Business code)
  reference: '11114' + Math.floor(Math.random() * 100), // input_ThirdPartyReference
  transaction: 'T12344CC', // input_TransactionReference
  amount: '10', // input_Amount
};

describe('Receive Money from a Mobile Wallet', function () {
  this.timeout(30000);
...
describe('Send Money to a Mobile Wallet', function () {
  this.timeout(30000);
...
describe('Send Money to a Business Account', function () {
  this.timeout(30000);
  it('Send Money successful', function (done) {
...
describe('Revert a Transaction', function () {
  it('Revert a Transaction successful', function (done) {
    clientRevert.revert(reversionData).then((r) => {
      expect(r.response.status).to.be.within(200, 201);
      done();
    }).catch((e) => {
      done(new Error("test case failed: " + e.response.outputError));
